...
2. `POST /api/auth/login` → JWT（有効期限7日）を取得
3. トークンは `localStorage.auth_token` に保存
4. 以降のすべての API リクエストに `Authorization: Bearer <token>` ヘッダーを付与
5. 401 レスポンス受信時は自動的に `/login` へリダイレクト

### ロール

| ロール | 権限 |
|--------|------|
| `admin` | 全テナントにアクセス可能、ユーザー管理 CRUD |
| `user` | `user_tenants` で紐付けられたテナントのみアクセス可能 |

### 認証 API `/api/auth`

| メソッド | パス | 認証 | 説明 |
|----------|------|------|------|
| POST | `/api/auth/login` | 不要 | ログイン（JWT 取得） |
| GET | `/api/auth/me` | 必要 | ログイン中のユーザー情報取得 |

### 管理 API `/api/admin`（admin ロールのみ）

| メソッド | パス | 説明 |
|----------|------|------|
| GET | `/api/admin/users` | ユーザー一覧 |
| POST | `/api/admin/users` | ユーザー作成 |
| PUT | `/api/admin/users/{id}` | ユーザー更新・テナント再割当 |
| DELETE | `/api/admin/users/{id}` | ユーザー削除（自分自身は不可） |
| GET | `/api/admin/tenants` | テナント一覧 |
| GET | `/api/admin/dashboard` | テナント別統計ダッシュボード |

**GET `/api/admin/dashboard` レスポンス例**
...
[
  {
    ...
    "total_invoices": 42,
    "status_counts": { "processed": 38, "approved": 4 },
    "ai_processed": 40,
    "input_tokens": 120000,
    "output_tokens": 15000,
    "company_count": 10,
    "user_count": 3,
    "last_upload": "2026-05-09T12:00:00"
  }
]
```
...
`scripts/seed_admin.py` の `EMAIL` / `PASSWORD` / `NAME` を書き換えてから実行する。

### DB スキーマ（public）

| テーブル | 説明 |
|----------|------|
| `public.users` | ユーザー（id / email / password_hash / name / role / is_active） |
| `public.user_tenants` | ユーザー↔テナント紐付け（user_id / tenant_slug） |
...
```
ブラウザ
  └── /login または /static  (HTML / CSS / JS)
       └── API リクエスト (Authorization: Bearer + X-Tenant-Slug ヘッダー付き)
            └── FastAPI (localhost:8000)
                 ├── TenantMiddleware  ← JWT 検証・テナント識別（API ルートのみ）
                 ├── /api/auth        ← 認証
                 ├── /api/admin       ← ユーザー管理（admin のみ）
                 ├── /api/invoices    ← 請求書管理
                 ├── /api/masters     ← 会社マスタ
                 └── /api/approvals   ← 承認管理
...
| テーブル | スキーマ |
|----------|----------|
| tenants / users / user_tenants | public |
...
JWT 検証後、以下の優先順位でテナントを特定する。

1. `X-Tenant-Slug` リクエストヘッダー
...
docker compose exec db psql -U postgres -d invoices \
  -c "SELECT public.provision_tenant('acme', 'ACME株式会社');"
```
...
サイドバー
  ...
  ├── 会社マスタ (masters)
  ├── 承認管理 (approvals)
  ├── ダッシュボード (dashboard) ← admin のみ表示
  └── ユーザー管理 (users)       ← admin のみ表示
```
...
- **一覧テーブル**: ファイル名 / 文書種別 / ステータス / 発行者 / 請求書番号 / 合計金額 / 紐付け会社 / 登録日 / 操作
  - ステータスフィルタ・テキスト検索対応
  - ページネーション（1 ページ 20 件）
  - **CSV エクスポート**: 現在の絞り込み結果を CSV でダウンロード（モバイルでは非表示）
...
- モバイルでは CSV 関連ボタンを非表示、会社名・操作列のみ表示
...
### テナントダッシュボード画面（admin のみ）

- 全テナントのサマリー行（テナント数・総請求書数・累計トークン・総ユーザー数・総会社マスタ数）
- テナントごとのカード表示
  - 請求書数 / AI 処理済件数 / ユーザー数 / 会社マスタ数
  - 入力トークン / 出力トークン / 推定コスト（Claude Haiku 料金レート）
  - ステータス別バッジ（pending / processing / processed / approved / error）
  - 最終アップロード日
- レスポンシブ対応（2列 / 1列グリッド）

### ユーザー管理画面（admin のみ）

- ユーザー一覧（名前 / メール / ロール / テナント / ステータス）
- ページネーション（1 ページ 20 件）
- 新規ユーザー作成・編集・削除
- ロール選択（admin / user）
- テナント割り当て（ユーザーのみ・チェックボックス）
- モバイルではメール・テナント・ステータス列を非表示
...
| `/masters` | 会社マスタ |
| `/approvals` | 承認管理 |
| `/dashboard` | テナントダッシュボード（admin のみ） |
...
| `SECRET_KEY` | — | `change-me-in-production` | JWT 署名キー（本番では必ず変更）|
| `ACCESS_TOKEN_EXPIRE_MINUTES` | — | `10080`（7日）| JWT 有効期限（分）|
...
│   │   ├── approvals.py     # 承認 API
│   │   ├── auth.py          # 認証 API (login / me)
│   │   └── admin.py         # 管理 API (ユーザー CRUD / ダッシュボード)
│   ├── services/
│   │   ├── claude.py        # Claude AI 連携（抽出・照合・トークン計測）
│   │   ├── s3.py            # AWS S3 連携（アップロード・移動・署名付き URL）
│   │   └── auth_svc.py      # JWT 生成・検証・パスワードハッシュ
│   └── static/
│       ├── index.html       # SPA エントリポイント
│       ├── login.html       # ログインページ
│       ├── css/style.css    # スタイルシート
│       └── js/
│           ├── auth.js      # トークン管理・ログインガード
...
│           ├── dashboard.js # テナントダッシュボードロジック
│           └── users.js     # ユーザー管理画面ロジック
├── migrations/
│   ├── init.sql             # DB 初期化 SQL (テナントプロビジョニング関数含む)
│   └── add_auth.sql         # 認証用テーブル追加 (users / user_tenants)
├── scripts/
│   └── seed_admin.py        # 初回管理者ユーザー登録スクリプト
...
ALTER SCHEMA tenant_default RENAME TO tenant_dev;